...
import {  GnDigfl, user, radio } from "../../class/models/models";
//Providers
...
export class LoginPage {
    ...
    // this.CheckConnectionChanges().then(
      // async () => {
        ...
        this.InitSession().then(async()=>{
          ...
          this.appVersionService.checkUpdatesApp();
          this.events.publish("user:gnempre");
          this.networkService.initNetwork();
          this.GetTouchId();
        })
  ...
  doLogin(action: string, password: any) {
   if(this.appVersionService.checkUpdatesApp()){
      ...
      let promise= new Promise<boolean>((resolve,reject)=>{
        this.session.getEmpCodiSession().then(resp=>{
          if(resp==undefined || resp!=null){     
            this.session.setPartnerConnections(settings);
            this.SetEmpCodi(settings.EMP_CODI)
            this.SetConexiones(settings);                     
            this.logo = settings.CNX_LOGO;
            this.backgroundColor = settings.CNX_BACK;
            this.colorPri = settings.CNX_CPRI;
            this.colorSeg = settings.CNX_CSEG;
            this.colorTer = settings.CNX_CTER;
            this.fonClar = settings.CNX_FCLA;
            resolve(true);
          }
          else {
            this.logo = settings.CNX_LOGO;
